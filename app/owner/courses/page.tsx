...
export default async function OwnerCoursesPage() {
  ...
  const { data: courses } = await supabase
    .from("courses")
    .select("id, name, duration_min, default_price, default_back_rate, valid_from, valid_to, is_active")
  ...
  return (
    <main className="mx-auto max-w-2xl px-4 py-8 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Link href="/owner" className="text-sm text-muted-foreground hover:underline">
          ← ダッシュボード
        </Link>
      </div>
